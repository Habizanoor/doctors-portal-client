...
const BookingModal = ({ treatment, date, setTreatment, refetch }) => {
    ...
    const handleBooking = event => {
        ...
        console.log(treatment.name, slot, phone);
        const booking = {
            treatmentId: _id,
            treatment: name,
            date: formattedDate,
        ...
        fetch('http://localhost:5000/booking', {
            ...
            .then(data => {
                console.log(data);
                if (data.success) {
                    toast(`Apponitment is set, ${formattedDate} at ${slot}`)
                    
                }
                else {
                    toast.error(`Already have an apponitment on, ${data.booking?.date} at ${data.booking?.slot}`)
